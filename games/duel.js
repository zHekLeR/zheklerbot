...
async function duelLbStreak(stream) {
    try {
        let res = await helper.dbQueryPromise(`SELECT * FROM duelduel WHERE streak = (SELECT MAX(streak) FROM duelduel) LIMIT 1;`);
        let longStreak = await helper.dbQueryPromise(`SELECT * FROM duelduel WHERE longest = (SELECT MAX(longest) FROM duelduel) LIMIT 1;`);
...
export { duel, cancel, coward, accept, duelScore, duelLb, duelLbRatio, duelLbRatioLow, duelLbStreak };