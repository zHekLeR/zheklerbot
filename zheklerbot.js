...
app.get('/send/:channel/:hKills/:tKills/:o1Kills/:o2Kills', async (request, response) => {
  try {
    ...
    if (request.get('tstatus') && userIds[request.get('tname')] && userIds[request.get('tname')]["two_v_two"] && rows[0].perms.split(',').includes(request.params.channel.toLowerCase())) {
    ...
    if (request.get('o1status') && userIds[request.get('o1name')] && userIds[request.get('o1name')]["two_v_two"] && rows[0].perms.split(',').includes(request.params.channel.toLowerCase())) {
    ...
    if (request.get('o2status') && userIds[request.get('o2name')] && userIds[request.get('o2name')]["two_v_two"] && rows[0].perms.split(',').includes(request.params.channel.toLowerCase())) {
...
app.get('/brookescribers', async (request, response) => {
  try {
    ...
    let rows = await helper.dbQueryPromise('SELECT * FROM brookescribers;');
...
async function daily(username) {
  try {
    ...
    let midnight = (DateTime.now().setZone('America/Los_Angeles').startOf('day').toMillis()/1000) - userIds[username].time_offset;
...
async function gulag(username) {
  try {
    // Midnight of current day.
    let midnight = (DateTime.now().setZone('America/Los_Angeles').startOf('day').millisecond/1000) - userIds[username].time_offset;
